...
class GenerateShell extends MvcShell {
	...
	private $hyphenate_plugin_dir = true;
	...
	public function plugin($args) {
		...
		$plugin = $args[0];
		$this->hyphenate_plugin_dir = $this->get_hyphenate_plugin_dir_args($args);
	...
	private function generate_app($plugin, $hyphenate_plugin_dir = true) {
		...
		$plugin_hyphenized = ($hyphenate_plugin_dir) ? str_replace('_', '-', $plugin_underscored) : $plugin_underscored;
	...
	private function get_hyphenate_plugin_dir_args($args) {
		if (count($args) <= 1) {
			return true;
		} else {
			$value = $args[count($args)-1];
			if (strtolower($value) !== "--underscore") {
				return true;
			} else {
				return false;
			}
	...
	private function get_plugin_model_args($args) {
		...
		$args[0] = ($this->hyphenate_plugin_dir) ? str_replace('_', '-', $args[0]) : $args[0];
		return $args;
	}

	private function get_plugin_path($plugin_name) {
		$plugin_underscored = MvcInflector::underscore($plugin_name);
		$plugin_hyphenized = ($this->hyphenate_plugin_dir) ? str_replace('_', '-', $plugin_underscored) : $plugin_underscored;
